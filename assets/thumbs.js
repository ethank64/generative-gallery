/*
  Procedural thumbnail renderer.
  Draws a small on-brand sketch into a <canvas> so the grid looks alive even
  before you've captured real screenshots. Override any tile with a real image
  by setting `thumb` in the manifest.

  Usage:  GalleryThumbs.render(canvasEl, kind, seed)
*/
(function () {
  ...
  window.GalleryThumbs = {
    render: function (canvas, kind, seed) {
      try {
